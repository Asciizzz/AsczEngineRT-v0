...
public:
    ...
    __device__ static inline float rsqrt(float x) {
        float xhalf = 0.5f * x;
        int i = *(int*)&x;
        i = 0x5f3759df - (i >> 1);
        x = *(float*)&i;
    ...
    __device__ static inline float ACESFilm(float x) {
        return (x * (2.51f * x + 0.03f)) / (x * (2.43f * x + 0.59f) + 0.14f);
    }

    // Fast exponential approximation (GPU-friendly)
    __device__ static inline float exp(float x) {
        x = 1.0f + x / 256.0f;
        x *= x; x *= x; x *= x; x *= x;
        x *= x; x *= x; x *= x; x *= x;
        return x;
    ...
    __device__ static inline float log(float x) {
        int i = *(int*)&x;
        float y = i;
    ...
    __device__ static inline float sin(float x) {
    ...
    __device__ static inline float cos(float x) {
        return AzDevMath::sin(x + M_PI * 0.5f);
    }

    // Wang Hash (for procedural randomness in CUDA kernels)
    __device__ static inline unsigned int wangHash(unsigned int seed) {
    ...
    __device__ static inline unsigned int spatialHash(unsigned int x, unsigned int y, unsigned int z) {
    ...
    __device__ static inline void cosineSampleHemisphere(float u1, float u2, float& x, float& y, float& z) {
        float r = sqrtf(u1);
    ...
    __device__ static inline void uniformSampleSphere(float u1, float u2, float& x, float& y, float& z) {
    ...
    __device__ static inline float sobol(int index, int dimension) {
    ...
    __device__ static inline void fibonacciSphereSample(int i, int n, float& x, float& y, float& z) {
    ...
    __device__ static inline float smithG1(float NoV, float alpha) {
        float a = alpha * alpha;
        float b = NoV * NoV;
    ...
    __device__ static inline float ggxNDF(float NoH, float alpha) {
